({
    getData : function(component, event, helper) {
        let action = component.get("c.getPriorityItem");
        action.setCallback(this, function(response){
            var state = response.getState();
            if (state === "SUCCESS") {
                //debugger;
                console.log('response----',response.getReturnValue());
                let PriorityItem = response.getReturnValue().PriorityItem;
                component.set('v.PriorityItem', PriorityItem);
            ...
            try{
                ...
                cometd.handshake($A.getCallback(function(status) {
                    if (status.successful) {
                        var eventName = "/event/Update_Current_Priority__e";
                        var subscription =
